#!/bin/bash
# Reproduce the final individual-level sweep with the OpinionQA-selected
# config 133 on all three datasets and model seeds 0--4.
set -euo pipefail
...
export DATASETS="OpinionQA EEDI Twin-2K-500"
export SEEDS="0 1 2 3 4"
export VARIANTS="x_only x_one_llm one_logprob x_avg_llm x_all_llm"
export GPU_DEVICES="cuda:0 cuda:1 cuda:2 cuda:3 cuda:4 cuda:5 cuda:6 cuda:7"

export RESULT_ROOT_REL="individual_test_tuned_a1e-6_d08_fulltrain_seed0_4"
export LOG_DIR="logs/individual_test_tuned_a1e-6_d08_fulltrain_seed0_4"
...
exec bash "$SCRIPT_DIR/run_individual.sh"
