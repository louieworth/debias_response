...
###############################################################################
# Unified grouped-variant runner for OpinionQA.
#
# Writes one combined per-seed CSV under:
#   results/group/OpinionQA/
#
# Default output files depend on LLM_VECTOR_TRANSFORM:
#   raw    -> results/group/OpinionQA/aggreated_opinionqa_multisource_result_seed_<seed>.csv
#   sorted -> results/group/OpinionQA/aggreated_opinionqa_multisource_sorted_result_seed_<seed>.csv
#
...
DEFAULT_LLM_FIELDS="claude-3.5-haiku_norm,deepseek-v3_norm,gpt-3.5-turbo_norm,gpt-4o_norm,gpt-4o-mini_norm,gpt-5-mini_norm,llama-3.3-70B-instruct-turbo_norm,mistral-7B-instruct-v0.3_norm"
...
DEFAULT_RESULT_BASENAME="aggreated_opinionqa_multisource${RESULT_SUFFIX}_result.csv"
...
if [[ "$ONE_LOGPROB_ONLY" =~ ^[Yy]$ ]]; then
    ...
    DEFAULT_RESULT_BASENAME="aggreated_opinionqa_one_logprob_result.csv"
fi

DATASET=OpinionQA \
TRAIN_FILE="${TRAIN_FILE:-dataset/OpinionQA/aggreated/opinionqa_train.parquet}" \
TEST_FILE="${TEST_FILE:-dataset/OpinionQA/aggreated/opinionqa_test.parquet}" \
RESULT_SUBDIR="${RESULT_SUBDIR:-group/OpinionQA}" \
