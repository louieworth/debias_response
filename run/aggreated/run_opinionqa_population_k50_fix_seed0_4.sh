#!/bin/bash
# Re-run the OpinionQA population comparison after enforcing a common K=50
# response budget for Base LLM, Mean, HuMCal-Mean, and Vector.
set -euo pipefail
...
RESULT_ROOT_REL="population_one_significance_k50fix_seed0_4_precision17"
...
for seed in "${SEEDS[@]}"; do
    ...
    echo "[launch] OpinionQA seed=$seed device=cuda:$seed"
    (
        RESULT_SUBDIR="$RESULT_ROOT_REL/OpinionQA" \
        RESULT_BASENAME="population_OpinionQA.csv" \
...
for index in "${!PIDS[@]}"; do
    if wait "${PIDS[$index]}"; then
        echo "[OK] OpinionQA/seed${SEEDS[$index]}"
    else
        echo "[FAIL] OpinionQA/seed${SEEDS[$index]} (see $LOG_ROOT)"
        failed=$((failed + 1))
    fi
done

if (( failed > 0 )); then
    echo "$failed OpinionQA tasks failed."
    exit 1
fi

echo "All ${#PIDS[@]} corrected OpinionQA population tasks completed."
