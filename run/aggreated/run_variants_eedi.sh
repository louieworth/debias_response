...
###############################################################################
# Unified grouped-variant runner for EEDI.
#
# Writes one combined per-seed CSV under:
#   results/group/EEDI/
#
# Default output files depend on LLM_VECTOR_TRANSFORM:
#   raw    -> results/group/EEDI/aggreated_eedi_multisource_result_seed_<seed>.csv
#   sorted -> results/group/EEDI/aggreated_eedi_multisource_sorted_result_seed_<seed>.csv
#
...
DEFAULT_RESULT_BASENAME="aggreated_eedi_multisource${RESULT_SUFFIX}_result.csv"
...
if [[ "$ONE_LOGPROB_ONLY" =~ ^[Yy]$ ]]; then
    ...
    DEFAULT_RESULT_BASENAME="aggreated_eedi_one_logprob_result.csv"
fi
...
RESULT_SUBDIR="${RESULT_SUBDIR:-group/EEDI}" \
